...
class WeightedMovingFilter:
    ...
    def add_data(self, new_data):
        arr = np.asarray(new_data, dtype=float).reshape(-1)
        if arr.size != self._data_size:
            raise ValueError(
                f"[WeightedMovingFilter] input size mismatch: got {arr.size}, expected {self._data_size}, "
                f"shape={np.asarray(new_data).shape}"
            )

        if len(self._data_queue) > 0 and np.array_equal(arr, self._data_queue[-1]):
            return  # skip duplicate data
        
        if len(self._data_queue) >= self._window_size:
            self._data_queue.pop(0)

        self._data_queue.append(arr)
