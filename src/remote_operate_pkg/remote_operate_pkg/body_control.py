...
class JointControl(Node):
    ...
    def send_trajectory(self, target_positions):
        ...
        # 创建各控制器的轨迹消息
        body_trajectory = JointTrajectory()
        body_trajectory.joint_names = body_joints
        # 获取各关节在control_joints中的索引
        neck_indices = [self.control_joints.index(j) for j in body_joints]
        # 线性插值
        for i in range(self.steps + 1):
            ...
            body_point.positions = [interpolated_pos[i] for i in neck_indices]
            body_point.time_from_start = point.time_from_start

            # 添加到各自的轨迹
            body_trajectory.points.append(body_point)

        # 发布轨迹
        self.neck_publisher.publish(body_trajectory)
